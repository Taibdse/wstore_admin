<template>
    <div class="content">
        <h2 class="my-page-header">{{ insertCategory ? 'Insert Category' : 'Update Category' }}</h2>
         <md-card class="my-card">
            <md-card-header :data-background-color="insertCategory ? 'green' : 'orange'" >
                    ...
                    <form class="md-layout" @submit.prevent="saveCategory">
                        <md-card class="">
                            <md-card-content>
                                <div class="md-layout md-gutter">
                                    <div class="md-layout-item sm-size-100">
                                        <md-field >
                                            ...
                                            <md-input name="name" id="name" v-model="category.name" @input="setCategorySlug" />
                                        </md-field>
                                    </div>
                                    ...
                                    <div class="md-layout-item sm-size-100">
                                        <md-field>
                                            ...
                                            <md-input name="slug" id="slug"  v-model="category.slug" disabled />
                                        </md-field>
                                    </div>
                                ...
                                <div class="md-layout md-gutter">
                                    <div class="md-layout-item md-size-100">
                                        <md-checkbox v-model="category.active">Active category</md-checkbox>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        getCategoryDetails: async function(){
            if(this.$route.path.indexOf('/categories/insert') > -1){
                ...
                const categorySlug = this.$route.params.categorySlug;
                 try {
                    const res = await CategoryService.getCategoryBySlug(categorySlug);
        ...
        setCategorySlug: function(value){
            this.category.slug = convertStringToSlug(value);
        },
    },
    async created(){
